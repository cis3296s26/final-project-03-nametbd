//Dashboard jobs module. This file loads profile summary information and paginated job results for the dashboard view.


(function () {
    // Small DOM getter helpers keep repeated element lookups readable.
    ...
    // Build a display-friendly full name with a fallback when profile data is incomplete.
    function getFullName(profile) {
        return `${profile?.firstName || ''} ${profile?.lastName || ''}`.trim() || 'Your Profile';
    }

    // Derive avatar initials from the profile name when no photo exists.
    ...
    // Prefer the uploaded profile photo; otherwise render a styled initials avatar.
    ...
    // Count how many saved jobs progressed to applications/interviews for summary cards.
    ...
    // Load profile and saved-job data in parallel, then fill the dashboard profile widget.
    ...
    // Convert numeric salary fields into readable card text.
    ...
    // Update the paging label and disable/enable navigation buttons as needed.
    ...
    // Build the visible set of dashboard job cards for the current page.
    ...
    // Fetch one page of dashboard jobs, then refresh the list/count/status UI.
    ...
    // Wire up page-level listeners and kick off the first data load.
