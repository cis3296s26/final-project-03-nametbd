(function () {
    // Allowed status values shown in the saved-jobs status dropdown.
    const statuses = ['SAVED', 'APPLIED', 'INTERVIEW', 'OFFER', 'REJECTED', 'WITHDRAWN'];

    // DOM getters for the saved-jobs page and shared dashboard view.
    ...
    // Format salary ranges consistently across saved-job cards.
    ...
    // Convert a search/listing result into the payload expected by the saved-jobs endpoint.
    ...
    // Build the richer card layout used when saved jobs are shown inside the dashboard.
    ...
    // Build the simpler list-item layout used on the standalone saved-jobs page.
    ...
    // Fetch saved jobs from the backend and render them into the active page layout.
    ...
    // Inject a reusable “Save Job” button into a result card/listing.
    ...
    // Expose saved-job helpers for other modules like jobs-search.
    window.SavedJobs = {
        addSaveButton,
