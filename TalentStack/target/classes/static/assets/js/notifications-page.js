(function () {
    ...
    function cardHTML(notification) {
        ...
        const actions = (notification.actions || []).map((action) => (
            action === 'accept'
                ? `<button class="btn-accept" type="button" data-action="accept" data-id="${notification.id}">Accept</button>`
                : `<button class="btn-decline" type="button" data-action="decline" data-id="${notification.id}">Decline</button>`
        )).join('');

        return `<div class="notif-card ${notification.unread ? 'unread' : ''}" id="notif-${notification.id}" data-id="${notification.id}">
            ...
            <button class="notif-dismiss" type="button" title="Dismiss" aria-label="Dismiss notification" data-action="dismiss" data-id="${notification.id}">&times;</button>
        </div>`;
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('undoDismissBtn')?.addEventListener('click', undoDismiss);
        ...
        feed.addEventListener('click', (event) => {
            ...
            if (actionButton instanceof HTMLElement) {
                ...
                if (actionButton.dataset.action === 'dismiss') {
                    event.stopPropagation();
                    dismiss(id);
                    return;
                }

                if (actionButton.dataset.action === 'accept' || actionButton.dataset.action === 'decline') {
                    event.stopPropagation();
                    handleAction(id, actionButton.dataset.action);
                    return;
                }
            }
